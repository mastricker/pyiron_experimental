...
class TestHSLineProfiles(TestWithCleanProject):
    ...
    def test_set_signal(self):
        ...
        with self.subTest("intended use case"):
            ...
            self.assertEqual(
                self.job.input.signal.axes,
                list(signal.axes_manager.as_dictionary().values()),
            )
            self.assertTrue(np.array_equal(self.job.input.signal.data, signal.data))
            self.assertDictEqual(
                self.job.input.signal.metadata, signal.metadata.as_dictionary()
            )
            self.assertDictEqual(
                self.job.input.signal.original_metadata,
    ...
    def test_hs(self):
        data = self.job.hs.load(
            os.path.join(self.project.path, "../../notebooks/experiment.emd")
        )
    ...
    def test_static_workflow(self):
        ...
        with self.subTest("Output line 0"):
            ...
            self.assertTrue(
                np.array_equal(output["x"], [0, 50]),
                msg=f"Expected {[0, 50]} but got {output['x']}.",
            )
            self.assertTrue(
                np.array_equal(output["y"], [10, 10]),
                msg=f"Expected {[10, 10]} but got {output['y']}.",
            )
        ...
        with self.subTest("Output line 1"):
            ...
            self.assertTrue(
                np.array_equal(output["x"], [50, 50]),
                msg=f"Expected {[50, 50]} but got {output['x']}.",
            )
            self.assertTrue(
                np.array_equal(output["y"], [0, 50]),
                msg=f"Expected {[0, 50]} but got {output['y']}.",
            )
